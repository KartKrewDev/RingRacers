...
struct Gles2Pipeline : public rhi::Pipeline {
	...
	explicit Gles2Pipeline(GLuint vertex_shader,
						   GLuint fragment_shader,
						   GLuint program,
						   const rhi::PipelineDesc& desc) noexcept
...
Gles2Rhi::Gles2Rhi(std::unique_ptr<Gles2Platform>&& platform)
...
rhi::Handle<rhi::Texture> Gles2Rhi::create_texture(const rhi::TextureDesc& desc, srb2::rhi::PixelFormat data_format, tcb::span<const std::byte> data) {
	SRB2_ASSERT(graphics_context_active_ == false);
...
void Gles2Rhi::update_texture(Handle<Texture> texture, Rect region, srb2::rhi::PixelFormat data_format, tcb::span<const std::byte> data) {
	SRB2_ASSERT(graphics_context_active_ == false);
	...
	glTexSubImage2D(GL_TEXTURE_2D, 0, region.x, region.y, region.w, region.h, format, type, reinterpret_cast<const void*>(data.data()));
}
...
rhi::Handle<rhi::Pipeline> Gles2Rhi::create_pipeline(const PipelineDesc& desc) {
	...
	if (is_compiled == GL_FALSE) {
		...
		glDeleteShader(vertex);
		throw std::runtime_error(std::string("Fragment shader compilation failed: ") + std::string(compile_error.data()));
	}
	...
	if (desc.uniform_input.enabled_uniforms.size() + desc.sampler_input.enabled_samplers.size() != static_cast<GLuint>(active_uniform_total)) {
		glDeleteProgram(program);
		glDeleteShader(fragment);
		glDeleteShader(vertex);
		std::string ex_msg("Pipeline's enabled uniform count (uniforms + samplers) does not match the linked program's total: ");
...
void Gles2Rhi::begin_render_pass(Handle<GraphicsContext> ctx, const RenderPassBeginInfo& info) {
	...
	if (fb_itr == framebuffers_.end()) {
		...
		fb_itr = framebuffers_.insert(
			{Gles2FramebufferKey {info.color_attachment, info.depth_attachment}, static_cast<uint32_t>(fb_name)}).first;
...
void Gles2Rhi::bind_pipeline(Handle<GraphicsContext> ctx, Handle<Pipeline> pipeline) {
	...
	if (desc.color_attachment.blend) {
		...
		glEnable(GL_BLEND);
		glBlendFuncSeparate(map_blend_factor(bl.source_factor_color),
	...
	glColorMask(desc.color_attachment.color_mask.r ? GL_TRUE : GL_FALSE,
...
void Gles2Rhi::update_bindings(Handle<GraphicsContext> ctx, const UpdateBindingsInfo& info) {
	...
	for (size_t i = 0; i < pl.desc.vertex_input.attr_layouts.size(); i++) {
		...
		glVertexAttribPointer(gl_attr_location, vertex_attr_size, vertex_attr_type, GL_FALSE, buffer_layout.stride, reinterpret_cast<const void*>(vertex_buffer_offset + attr_layout.offset));
	}
...
void Gles2Rhi::draw_indexed(Handle<GraphicsContext> ctx,
							uint32_t index_count,
							uint32_t first_index,
							uint32_t vertex_offset) {
	SRB2_ASSERT(graphics_context_active_ == true && graphics_context_generation_ == ctx.generation());
	glDrawElements(map_primitive_mode(current_primitive_type_), first_index, GL_UNSIGNED_SHORT, reinterpret_cast<const void*>(index_buffer_offset_));
}
